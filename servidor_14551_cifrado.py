#!/usr/bin/env python3
"""
Servidor UDP dedicado para puerto 14551 - mensajes cifrados ASCON
"""
...
def main():
    print("🔒 === Servidor UDP Puerto 14551 - Mensajes Cifrados ASCON ===")
    
    # Crear socket UDP servidor para 14551
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    
    try:
        server_address = ('127.0.0.1', 14551)
        sock.bind(server_address)
        print(f"✅ Servidor UDP iniciado en {server_address}")
        print("🎯 Esperando mensajes cifrados de ArduCopter...\n")
        ...
        while True:
            try:
                ...
                if msg_info and msg_info['encrypted']:
                    encrypted_count += 1
                    ...
                    # Verificar payload cifrado (debería ser no-ASCII)
                    payload_start = 10
                    payload_end = payload_start + msg_info['payload_len']
                    if payload_end <= len(data):
                        payload = data[payload_start:payload_end-2]  # -2 para CRC
                        ...
                        # Mostrar últimos 16 bytes (debería ser el tag ASCON)
                        if len(payload) >= 16:
                            tag_start = len(payload) - 16
                            tag = payload[tag_start:tag_start+16]
                            tag_hex = ' '.join(f'{b:02X}' for b in tag)
                            print(f"   🏷️  Tag ASCON (últimos 16): {tag_hex}")
                    
                    print(f"   🎯 Total: {total_count}, Solo cifrados: {encrypted_count}")
                    print("=" * 80)
                    
                elif msg_info:
                    print(f"⚠️  Mensaje NO cifrado recibido en puerto 14551: "
                          f"msgid={msg_info['msgid']}, flags=0x{msg_info['incompat_flags']:02X}")
                else:
                    print(f"❌ Datos inválidos recibidos: {len(data)} bytes")
        ...
        if total_count > 0:
            print(f"   📈 Porcentaje cifrado: {encrypted_count/total_count*100:.1f}%")
